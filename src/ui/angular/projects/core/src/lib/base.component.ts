...
export class MddsBaseComponent implements MddsBaseComponentInterface {
    ...
    constructor(
        ...
        if (injector) {
            this.commonService = injector.get<MddsCommonService>(MddsCommonService as Type<MddsCommonService>);
        }
    }
    ...
    public onGotoPage(p: number): void {
        if (p > this.totalPages || p < 1) { return; }
        this.routeToPage(p);
    }
    ...
    public formatReferenceField(field: any, fieldName: string ): any {
        ...
        if (typeof field === 'string') {
            ...
            if (refDetail && (typeof refDetail === 'object')) { field = refDetail; } else {
                id = field;
    ...
    public deFormatReference(detail: any ): any {
        for (const fnm of this.referenceFields) {
            if (typeof detail[fnm] !== 'object') { // not defined
                ...
                if (typeof id !== 'string') { delete detail[fnm]; } else { detail[fnm] = id; }
            }
        }
        return detail;
    }
    ...
    public formatDateField(field: string): any {
        ...
        const value = fmt.replace(/yyyy/ig, yyyy.toString()).
        ...
        */
        ...
        return {date: { day: d, month: M, year: yyyy}, value, from: undefined, to: undefined, pop: false,
    ...
    public deFormatDateField(date: any): string {
        ...
        if (typeof yyyy !== 'number' || typeof M !== 'number' || typeof d !== 'number') { return null; } else {
    ...
    public deFormatDate(detail: any ): any {
        for (const fnm of this.dateFields) {
            if (typeof detail[fnm] !== 'object') { // not defined
                ...
                if (! detail[fnm].date) { delete detail[fnm]; } else {
                    const dateStr = this.deFormatDateField(detail[fnm].date);
                    if (!dateStr) { delete detail[fnm]; } else { detail[fnm] = dateStr; }
                }
            }
        }
        return detail;
    ...
    public deFormatArrayMultiSelection(detail: any ): any {
        for (const fnm of this.multiSelectionFields) {
            if (typeof detail[fnm] !== 'object') { // not defined
                ...
                if (! detail[fnm].selection) { delete detail[fnm]; } else {
                    const selectArray = [];
                    for (const e of this.enums[fnm]) {
                        if (detail[fnm].selection[e]) { selectArray.push(e); }
                    }

                    if (selectArray.length > 0) { detail[fnm] = selectArray; } else { delete detail[fnm]; }
                }
            }
        }
        return detail;
        ...
        for (const f of this.mapFields) {
            ...
            if (typeof detail[fnm] !== 'object') { // not defined
                ...
                if (! detail[fnm].selection) { delete detail[fnm]; } else {
                    ...
                    //   }
                    // }
                    if (Object.keys(selectMap).length > 0) { detail[fnm] = selectMap; } else { delete detail[fnm]; }
                }
            }
        }
        return detail;
    ...
    public stringifyField(field: any): string {
        ...
        if (typeof field === 'number' ||
            ...
            typeof field === 'bigint') { return String(field); }
    ...
    public extraInfoPopulate() {
        for (const fieldDef of this.mapFields) {
          ...
          if (mapKeyType === 'ObjectId') {
            ...
            const id = this.detail[keyRefName] ? this.detail[keyRefName]._id : undefined;
            if (!id) { continue; }
        ...
        for (const field of Object.keys(d2)) {
            ...
            if (this.stringFields.indexOf(field) >= 0 && listCategoryFields.includes(field)) { // put category field to "and"
                ...
                if (this.multiSelectionFields.includes(field)) {
                    ...
                    const t = this.formatArrayMultiSelectionField(d2[field], this.enums[field]);
                    valueToShow = t.value;
        ...
        */
        ...
        if (context && context.$and) {
            ...
            for (const sub of context.$and) {
                if ('$and' in sub) { cachedAnd = sub.$and; } else if ('$or' in sub) { cachedOr = sub.$or; }
            }
    ...
    public populateList(): EventEmitter<any> {
        ...
        if (cachedPage) {
            newPage = cachedPage;
        } else if (urlPage) { newPage = urlPage; } else { newPage = 1; }
        ...
        this.service.getList(newPage, this.perPage, searchContext, this.listSortField, this.listSortOrder,
            ...
            for (const c of cateGroup) {
                if (c.listCategoryField && !c.categoryProvided) {
                    ...
                    c.categoriesOut = c.categories.map((x: any) => this.formatDetail(x));
            ...
            if (this.majorUi) {
                ...
                this.router.navigate(['.'], {relativeTo: this.route, queryParams, queryParamsHandling: 'merge', }); // update the url
            }

            if (this.isDropdownList) {
                this.dropdownItems = this.list.map(x => ({displayName: this.stringify(x), id: x._id}) );
            }
    ...
    public toggleListSort(field: string, fieldDisplay: string): void {
        if (field !== this.listSortField) {
            ...
            if (this.listSortOrder === 'asc') { this.listSortOrder = 'desc'; } else { this.listSortOrder = 'asc'; }
        }
    ...
    public onExport(): void {
        ...
        this.service.getList(0, 0, searchContext, this.listSortField, this.listSortOrder,
            ...
            this.associationField, actionType, null, this.ignoreField).subscribe(
                ...
                setTimeout(() => { window.URL.revokeObjectURL(downloadUrl); }, 100); // cleanup temporary url blob
            },
            this.onServiceError
        );
    }

    public onActionBase(actionType: any, actionData: any, succMessage: any, resultFields: string[]): void {
        const searchContext = this.getFromStorage('searchContext');
        this.loadUIFromCache();
    ...
    public clearValueFromArrayField(field: string, idx: number): void {
        if (this.detail[field].selection) {
            this.detail[field].selection = this.detail[field].selection.filter((x: any, i: number) => i !== idx );
            // check if any info needs to change after clear certain values;
            this.extraInfoPopulate();
        }
    }
    ...
    public onRefSelect(fieldName: string) {
        ...
        if (!componentRef) {
            ...
            const componentFactory = this.componentFactoryResolver.resolveComponentFactory(comType);
        ...
        this.componentSubscription = componentInstance.done.subscribe( (val: any) => {
            if (val) {
    ...
    */
    ...
    onEdtiorPreview(editorName: string) {
        if (this.textEditors) {
            this.textEditors.forEach( (editor: { name: string; preview: () => void; }) => {
    ...
    public onActionBaseEvent(event: { actionType: any; actionData: any; succMessage: any; resultFields: any; }) {

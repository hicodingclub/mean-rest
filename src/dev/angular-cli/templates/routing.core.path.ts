...
<%_ for (let sch_name in schemaMap) {
  let schm = schemaMap[sch_name]; let api = schm.api; let detailType = schm.detailType;
  %>
  <%_ if (api.includes("L")) {%>import { <%-schm.SchemaName%>ListGeneralComponent } from './<%-schm.schemaName%>/<%-schm.schemaName%>-list/<%-schm.schemaName%>-list-general.component';<%}%>
...
<%}%>
<%_ for (let sch_name in schemaMap) {
  let schm = schemaMap[sch_name]; let api = schm.api;
  %><%if (schm.sFeatures.hasRef) {%>
  <%_ if (api.includes("L")) {
...
export const <%-schm.schemaName%>RoutingCorePath = [
    <%if (api.includes("L")) {%>{path: 'list', component: <%-schm.SchemaName%>ListGeneralComponent<%
